...
export function validateCommand(command: string, firstParam: string|undefined, secondParam: number|undefined) {
...
export function validateSepoliaEnvironment() {
    ...
    const ownerPriv = process.env['SEPOLIA_CONTRACT_OWNER_PRIVATE_KEY']
    const isValidAddress = validateAddress(contractAddress);

    if (!apiKey || !contractAddress || !ownerPriv || !isValidAddress) {
...
export function validateLocalhostEnvironment() {
    ...
    const ownerPriv = process.env['LOCALHOST_CONTRACT_OWNER_PRIVATE_KEY']
    const isValidAddress = validateAddress(contractAddress);

    if (!nodeUrl || !contractAddress || !ownerPriv || !isValidAddress) {
        throw new InvalidEnvironmentDataException('Invalid .env file.');
    }
}
